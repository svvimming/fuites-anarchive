...
class QueueManager:
    ...
    def __init__(
        ...
        # Batch tracking
        self._batch_buffers: Dict[str, List[ChunkData]] = {}  # batch_id -> buffered chunks
    ...
    def receive_chunks(self) -> None:
        """
        Receive processed chunks from background, buffer by batch, and spawn
        complete batches as explosion layouts.

        Drains the entire results queue each frame (buffering is cheap).
        Physics bodies are only created when a batch is complete, spawning
        all chunks in one frame so the "whole" is briefly visible.
        """
        ...
        while True:
            ...
            if isinstance(item, BatchComplete):
                ...
                if batch_id not in self._batch_buffers:
                    self._batch_buffers[batch_id] = []
                self._batch_buffers[batch_id].append(item)
            else:
