...
// ///////////////// Convert text into a downloa//dable file and prompt download
...
// //////////////////////////////////////////////////////////// ConnectWebsocket
const ConnectWebsocket = (config, app) => (instance, next) => {
  return new Promise((resolve) => {
    instance.socket = app.$nuxtSocket(config.socketOptions)
    instance.socket.on('disconnect', async () => {
      await app.$awaitServerReconnect()
...
export default ({ $config, app }, inject) => {
  ...
  inject('convertHSLtoRGB', ConvertHSLtoRGB)
  inject('convertRGBtoHex', ConvertRGBtoHex)
