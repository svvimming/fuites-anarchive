...
for (let i = 0; i < mongoInstances.length; i++) {
  ...
  MC.socket.listeners.push({
    name: `${instance}|module|file-upload-chunk|payload`,
    async handler (data) {
      try {
        ...
        const file = Fs.createWriteStream(`${TMP_UPLOADS_DIR}/${instance}/${fileId}`, { flags: 'a' })
        file.write(chunk)
        file.end()
        if (data.place > data.goal) {
          await Fs.move(`${TMP_UPLOADS_DIR}/${instance}/${fileId}`, `${UPLOADS_DIR}/${instance}/${fileId}.${fileExt}`)
          const upload = await MC.mongoInstances[instance].model.Upload.findById(fileId)
          upload.upload_status = 1
          await upload.save()
          // File upload complete
          return socket.emit(`${instance}|module|file-upload-complete|payload`)
        }
        ...
        console.log(`[websocket: ${instance}|module|file-upload-chunk|payload]`)
        console.log(e)
      }
    }
  })
