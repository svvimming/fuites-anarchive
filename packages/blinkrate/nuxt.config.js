export default {
  ...
  publicRuntimeConfig: {
    frontendUrl: (function () {
      ...
      let uri = 'https://localhost:2010'
      switch (env) {
        case 'stable': uri = 'https://blinkrate-stable.fuit.es'; break
        case 'production': uri = 'https://blinkrate.fuit.es'; break
      } return uri
    }()),
    backendUrl: (function () {
      const env = process.env.SERVER_ENV
      ...
      switch (env) {
        ...
        case 'production': uri = 'https://blinkrate.fuit.es/api'; break
      } return uri
    }()),
  ...
  server: {
    port: (function () {
      const env = process.env.SERVER_ENV
      let port = 2010 // development
      switch (env) {
        case 'stable': port = 2011; break
        case 'production': port = 2012; break
      } return port
    }()),
