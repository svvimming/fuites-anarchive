export default {
  ...
  publicRuntimeConfig: {
    frontendUrl: (function () {
      ...
      let uri = 'https://localhost:2007'
      switch (env) {
        case 'stable': uri = 'https://interweave-stable.fuit.es'; break
        case 'production': uri = 'https://interweave.fuit.es'; break
      } return uri
    }()),
    backendUrl: (function () {
      const env = process.env.SERVER_ENV
  ...
  server: {
    port: (function () {
      const env = process.env.SERVER_ENV
      let port = 2007 // development
      switch (env) {
        case 'stable': port = 2008; break
        case 'production': port = 2009; break
      } return port
    }()),
